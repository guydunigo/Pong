...
Circle::Circle(float posX, float posY, float _alpha, float _radius, int r,
               int g, int b, float celX, float celY, float _rotZ, bool _isFixed)
    : Shape(posX, posY, r, g, b, celX, celY, _isFixed), radius(_radius),
      rotZ(_rotZ), alpha(_alpha) {
    updateMass();
}
...
void Circle::step(float dt, float gravityX, float gravityY,
                  std::vector<Rectangle *> &rects, std::vector<Circle *> &circs,
                  std::vector<Triangle *> &trigs, float width, float height) {
    if (!isFixed) {
        float massO(0), radO(0), gravity[2], norm[2], tang[2], new_pos[2],
            new_cel[2], other_cel[2], new_rotZ(rotZ), a(0), b(0), c(0), d(0);
        unsigned int i(0);
        Circle *other;
        Rectangle *rec;
        Triangle *trig;
        ...
        if (new_pos[0] < 0 || new_pos[0] + radius * 2 > width ||
            new_pos[1] < 0 || new_pos[1] + radius * 2 > height) {
            points += WALLS_MALUS;
            if (new_pos[0] < 0) { // Left
                norm[0] = 1;
                norm[1] = 0; // tang = [0,-1]
            ...
            } else if (new_pos[0] + radius * 2 > width) { // Right
                norm[0] = -1;
                norm[1] = 0; // tang = [0, 1]
            ...
            } else if (new_pos[1] + radius * 2 > height) { // Bottom
                norm[0] += 0;
                norm[1] += -1; // tang = [-1,0]
            ...
            new_rotZ = (2 * radius * new_rotZ -
                        5 * (new_cel[0] * tang[0] + new_cel[1] * tang[1])) /
                       (7 * radius);
            a = (-1) * BOUNCE_COEF *
                (new_cel[0] * norm[0] + new_cel[1] * norm[1]);
            for (i = 0; i < 2; i++) {
        ...
        for (i = 0; i < rects.size(); i++) {
            ...
            if (c < radius + sqrtf(width * width + height * height) / 2) {
                // Vertical collisions :
                if (new_pos[0] + radius > rec->getX() &&
                    new_pos[0] + radius < rec->getX() + rec->getWidth()) {
                    // Collision on a (moving) plane
                    if (new_pos[1] + 2 * radius > rec->getY() &&
                        new_pos[1] < rec->getY()) {
                        collision = true;
                        // Top collision
                        norm[0] = 0;
                ...
                // Horizontal collisions :
                else if (new_pos[1] + radius > rec->getY() &&
                         new_pos[1] + radius < rec->getY() + rec->getHeight()) {
                    // Collision on a (moving) plane
                    if (new_pos[0] + 2 * radius > rec->getX() &&
                        new_pos[0] < rec->getX()) {
                        collision = true;
                        // Left collision
                        norm[0] = -1;
                    ...
                    if ((a > 0 && b < 0)) {
                        ...
                        c = sqrtf(a * a + b * b); // Norm of this vector
                        // Collision on an corner (like a little sphere or a 45°
                        // plane ?)
                        if (c < radius) {
                            collision = true;
                        }
                ...
                if (collision) {
                    ...
                    other_cel[0] =
                        (rec->getCelX() * norm[0] + rec->getCelY() * norm[1]) *
                        norm[0];
                    other_cel[1] =
                        (rec->getCelX() * norm[0] + rec->getCelY() * norm[1]) *
                        norm[1];
                    ...
                    a = (-1) * B_R_BOUNCE_COEF *
                        (new_cel[0] * norm[0] + new_cel[1] * norm[1]);
                    b = (1 + B_R_BOUNCE_COEF * (other_cel[0] * norm[0] +
                                                other_cel[1] * norm[1]));
                    for (i = 0; i < 2; i++) {
                        new_cel[i] =
                            (a + b) * norm[i] - rotZ * radius * tang[i];
                    }
                }
            }
        }
        ...
        for (i = 0; i < trigs.size(); i++) {
            ...
            if (!trig->getIsOff()) {
                ...
                if (c < trig->getRadius() + radius) {
                    trig->effects(this);
                    points += TRIG_BONUS; // Always gain a bonus with triangles
                                          // (even for bad ones)
                }
            }
        }
        ...
        for (i = 0; i < circs.size(); i++) {
            if (circs[i] != this) {
                ...
                c = sqrtf(a * a + b * b); // Norm of this vector
                d = c - radius -
                    other->getRadius(); // Depth of penetration of the two balls
                ...
                if (d < 0) {
                    ...
                    d = 2 / (mass + massO);
                    new_cel[0] =
                        (c * new_cel[0] + massO * d * other->getCelX()) *
                        BOUNCE_COEF;
                    new_cel[1] =
                        (c * new_cel[1] + massO * d * other->getCelY()) *
                        BOUNCE_COEF;
                    other->setCelX((-c * other->getCelX() + mass * d * a) *
                                   BOUNCE_COEF);
                    other->setCelY((-c * other->getCelY() + mass * d * b) *
                                   BOUNCE_COEF);
                    new_pos[0] += norm[0];
                    new_pos[1] += norm[1];
