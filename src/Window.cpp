...
void Window::display(void) {
    ...
    play.setPosition((_win->getSize().x - play.getGlobalBounds().width) / 2,
                     (_win->getSize().y / 2 - play.getGlobalBounds().height));
    play.setString("Play !");

    quit.setFont(font);
    quit.setCharacterSize(50);
    quit.setFillColor(sf::Color(255, 255, 255));
    quit.setPosition((_win->getSize().x - play.getGlobalBounds().width) / 2,
                     (_win->getSize().y / 2 + play.getGlobalBounds().height));
    quit.setString("Quit");
    ...
    gameOver.setPosition(
        (_win->getSize().x - play.getGlobalBounds().width) / 2,
        (_win->getSize().y / 2 + play.getGlobalBounds().height / 2));
...
void Window::moveAll(float dx, float dy) { world->moveAll(dx, dy); }

void Window::drawAll() const { world->drawAll(_win); }

void Window::addRect(Rectangle *rect) { world->addRect(rect); }

void Window::addCirc(Circle *circ) { world->addCirc(circ); }
